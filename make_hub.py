...
if args.bam and args.display_bam_as_bam:
    ...
    for bam_file in args.bam:
        ...
        subprcs_args = [samtools, "sort", "-@",
                        str(args.threads), bam_file, "-o", bam_sorted_file]
        run_simple_process(subprcs_args)
        bam_index_file = hub_dir + "rnaseq_" + str(bam_index) + ".s.bam.bai"
        subprcs_args = [samtools, "index", "-@", str(args.threads),
...
''' Creating BRAKER genes track '''

if args.braker_genes:
    print('Generating BRAKER genes track...')
    check_trackDb(trackDb_file, "BRAKER predictions with hints")
    ucsc_file = tmp_dir + "braker.gtf"
    aug2ucsc_gtf(args.braker_genes, ucsc_file)
    visibility_counter, visibility = set_visibility(visibility_counter)
    col_idx, this_color = set_color(col_idx, rgb_cols)
    make_gtf_track(trackDb_file, ucsc_file, ChromSizes_file, "BRAKER",
                   "BRAKER predictions with hints",
                   this_color, visibility, args.no_genePredToBigGenePred, True)
    html_file = hub_dir + "BRAKER.html"
    write_trackDescription(html_file, "BRAKER", "This gene prediction track with BRAKER predictions with hints " +
                           "was generated by <a href=\"https://github.com/Gaius-Augustus/MakeHub\">" +
                           "make_hub.py</a> using <a href=\"http://hgdownload.soe.ucsc.edu/admin/exe\">UCSC tools</a>.</p>",
                           args.email, this_color)
